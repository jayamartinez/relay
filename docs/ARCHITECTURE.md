...
All durable coordinator state lives in SQLite. Runtime object fields are not relied on after hibernation. Short HTTP actions are serialized inside `blockConcurrencyWhile`; responses are bounded, and network request bodies are streamed with a 2 MB limit. Backoff is exponential with jitter, capped near five minutes. A one-minute alarm watchdog wakes a terminated worker. No socket is opened for anonymous, draft or pending devices, or while paused.
