...
The controller has explicit `UNINITIALIZED`, `LOADING_LOCAL_STATE`, `WAITING_FOR_BROWSER_RESTORE`, `FETCHING_CANONICAL_STATE`, `RECONCILING`, `LIVE`, and `STOPPED` phases. Browser changes upload only in `LIVE`. URL commits are coalesced independently per tab for 200 ms; close bursts use a 1.2-second classification window. Structural events trigger bounded queries. Relay does not subscribe to activation, focus, scrolling or geometry.

On a same-session worker restart, persisted mappings remain available. In a replacement session, hydration observes native restore progress until the browser topology is stable and event callbacks are quiet. Complete restores settle quickly; partial restores receive a bounded grace period before Relay fills genuine canonical gaps. Hydration maps physical windows to existing canonical windows using prior mapping, exact fingerprints, unique overlap, then canonical window order. Duplicate URLs match by occurrence/order. Resume never invents window IDs; excess unmatched physical windows remain ignored until an explicit `LIVE` creation event. Missing startup resources are not deletion evidence. Explicit tab opens, closes, and navigations during the settling phase are adopted before current canonical differences are applied.
...
A bounded navigation circuit also tracks unexpected local navigation while a recent remote create/navigation expectation is active. Three such reversals for the same logical tab within 60 seconds automatically pause networking and retain queued edits, with a visible error. Explicit address-bar, bookmark, and back/forward commits release the old expectation and reversal history, so deliberate rapid navigation does not trip that old remote intent. This heuristic still cannot identify every redirect or conflicting-edit loop. The user must inspect the affected pages before resuming after a circuit pause.

The browser APIs and IndexedDB cannot participate in one atomic transaction. A crash between a browser creation and saving its returned ID can still leave an orphan/duplicate. Relay favors preserving such tabs. `isWindowClosing` plus a 1.2-second all-normal-windows query distinguishes a final/rapid browser exit from one-window deletion; `STOPPED` preserves prior mapping and canonical state. Chromium exposes no definitive “browser process is exiting” extension event, so unusually staggered closes beyond that window remain a limitation.
...
Signed HTTPS requests carry ciphertext mutations. A one-use, 30-second ticket upgrades an authenticated WebSocket; messages broadcast only a change hint or a signed revocation chain. Clients retrieve canonical encrypted data through authenticated requests. Every connected device sends `ping` each 25 seconds. The Durable Object auto-responds `pong` through the hibernation API, without running application code for that heartbeat. The alarm watchdog replaces sockets that remain connecting for more than 20 seconds or open without a message for more than 75 seconds. Socket change hints use a generation marker, so a notification received during restoration or reconciliation remains dirty until a later canonical pull acknowledges it.

All durable coordinator state lives in SQLite. Runtime object fields are not relied on after hibernation. Short HTTP actions are serialized inside `blockConcurrencyWhile`; responses are bounded, and network request bodies are streamed with a 2 MB limit. Reconnect delay starts at 750–1,000 ms, doubles, and caps at 22.5–30 seconds. Backoff resets only after a connection has received traffic for at least a minute. A one-minute alarm watchdog is installed before network work, including the first connection, and wakes a terminated worker. Retry failures also schedule a durable alarm. Socket callbacks are attached before persistence and reject callbacks from retired sockets. No socket is opened for anonymous, draft or pending devices, or while paused.

Transport failures while reading a response body, non-JSON 408/429/5xx responses, known browser-object races, aborted IndexedDB transactions, and expired authentication challenges recover automatically. Expired challenges are rejected and replaced; identity, signature, control-chain, schema, and other challenge validation remain fail-closed. Interrupted local loading retries the complete load without resetting identity. Browser evidence is retained after failed capture and during awaited startup mutations. Background browser work coalesces behind the serial queue and does not busy-reschedule while halted or hydrating.

Device projections turn canonical ordering keys (which can tie after offline edits) into contiguous browser positions without modifying signed canonical state. Existing physical tab bindings reserve their logical IDs before duplicate-URL fallback matching. Restore uses ordered signature indexes, and group adoption indexes exact logical member sets. Already-current browser/group state skips per-resource mutation work. Persist skips identical state and snapshots its input before encryption; required journal/intent/returned-ID durability boundaries remain immediate. Development runtime diagnostics expose request and write counts, pending tasks, backoff, revisions, remote dirty state, and safe error categories without browsing data.

See [the reliability hardening report](RELIABILITY-HARDENING.md) for measured workloads, regression coverage, remaining limits, and the manual test plan.
